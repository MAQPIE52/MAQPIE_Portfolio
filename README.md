...
```
.
├── index.html                  메인 (히어로 / 소개 / 경력 / 기술 / 프로젝트 / 연락처)
...
`period` / `role` / `summary` / `problem` / `approach` / `result` 는 없으면 해당 줄이나 블록이 그냥 빠집니다.
문제·접근·결과가 하나도 없으면 상세 페이지에 "상세 내용은 정리 중입니다" 안내가 표시됩니다.

현재 14건 중 **5건은 문제·접근·결과까지 채워져 있고, SK텔레콤 자동화 9건은 제목·기간·역할·요약까지만** 들어 있습니다.
...
- **SK텔레콤 자동화 9건의 문제 · 접근 · 결과** — 지금은 상세 페이지가 "정리 중" 으로만
  뜹니다. 한 줄 요약도 프로젝트 이름을 풀어쓴 수준이라 실제 내용에 맞게 다듬어야 합니다.
- `index.html` 의 **기술 스택** — 실제 사용 도구에 맞게 조정
- **프로젝트 썸네일** — `assets/img/projects/` 가 비어 있어 전부 자동 생성 SVG 로 나옵니다
- `assets/files/resume.pdf` — 현재 빈 파일

## 공개하지 않기로 한 것

포트폴리오 평가에 쓰이지 않고, 이름과 함께 공개되면 도용 위험이 있는 정보는
사이트에 넣지 않았습니다. 공개 저장소라 한 번 커밋하면 이력에 남는 점도 고려했습니다.
